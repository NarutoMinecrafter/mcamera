"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.PacketsService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const packets_repository_1 = require("./packets.repository");
const class_validator_1 = require("class-validator");
let PacketsService = class PacketsService {
    constructor(packetsRepository) {
        this.packetsRepository = packetsRepository;
    }
    async getPacketById(id) {
        const found = await this.packetsRepository.findOne(id);
        if (!found) {
            throw new common_1.NotFoundException(`Packet with id "${id}" not found`);
        }
        return found;
    }
    async getPacketByDeviceIIdentifier(identifier) {
        return this.packetsRepository.getPacketByDeviceIIdentifier(identifier);
    }
    async createPacket(createPacketDto) {
        const result = await class_validator_1.validate(createPacketDto);
        return this.packetsRepository.createPacket(createPacketDto);
    }
    async updatePacketStatus(id, status) {
        const packet = await this.getPacketById(id);
        packet.status = status;
        packet.lastPacketTime = new Date();
        await packet.save();
        return packet;
    }
    async updatePacketUpdateTime(id, newValue) {
        const packet = await this.getPacketById(id);
        packet.lastPacketTime = newValue;
        await packet.save();
        return packet;
    }
};
PacketsService = __decorate([
    common_1.Injectable(),
    __param(0, typeorm_1.InjectRepository(packets_repository_1.PacketsRepository)),
    __metadata("design:paramtypes", [packets_repository_1.PacketsRepository])
], PacketsService);
exports.PacketsService = PacketsService;
//# sourceMappingURL=packets.service.js.map